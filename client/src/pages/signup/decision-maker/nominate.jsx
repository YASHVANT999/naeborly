...
export default function DecisionMakerNominate() {
  ...
  const [salesReps, setSalesReps] = useState([{ name: "", email: "", company: "", referralReason: "" }]);

  const form = useForm({
    resolver: zodResolver(decisionMakerNominationSchema),
    defaultValues: {
      nominatedSalesReps: [{ name: "", email: "", company: "", referralReason: "" }]
    }
  });
  ...
  const saveNominationMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "Your sales rep nominations have been saved successfully."
      });
      setLocation("/signup/decision-maker/package");
    },
    onError: (error) => {
      console.error('Decision maker nomination save error:', error);
      toast({
        title: "Save Failed",
        description: error.message || "Failed to save nominations. Please try again.",
        variant: "destructive"
      });
    }
  });
  ...
  const onSubmit = (data) => {
    ...
    const validNominations = data.nominatedSalesReps.filter(rep => rep.name || rep.email);
    saveNominationMutation.mutate({ nominatedSalesReps: validNominations });
  };

  const addSalesRep = () => {
    const newSalesReps = [...salesReps, { name: "", email: "", company: "", referralReason: "" }];
  ...
  const skipStep = () => {
    toast({
      title: "Step Skipped",
      description: "You can nominate sales reps later from your dashboard."
    });
    setLocation("/signup/decision-maker/package");
  };
        ...
        <Card className="shadow-lg">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Nominate Sales Representatives</h1>
              <p className="text-gray-600">Help build our community by nominating quality sales reps (optional)</p>
            </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-6 mb-8">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h3 className="text-sm font-medium text-purple-900 mb-2">Earn Credits by Nominating</h3>
                  <p className="text-sm text-purple-700 mb-3">
                    For each sales rep you nominate who successfully joins and completes calls, you earn credits 
                    that can be used for premium features or extended call durations.
                  </p>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <FormControl>
                              <Textarea
                                ...
                                onChange={(e) => {
                                  field.onChange(e);
                                  const newSalesReps = [...salesReps];
                                  newSalesReps[index].referralReason = e.target.value;
                                  setSalesReps(newSalesReps);
                                }}
                              />
                            </FormControl>
                ...
                <div className="flex justify-between pt-6">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setLocation("/signup/decision-maker/availability")}
