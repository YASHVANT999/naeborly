...
export default function RepSuspensionManagement() {
  ...
  const filteredSuspensions = suspensions.filter(suspension => {
    const matchesSearch = suspension.repName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Rep Suspension Management</h1>
              <p className="text-gray-600 mt-1">Monitor and manage sales rep suspensions</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">
                    {suspensions.filter(s => s.isActive && new Date(s.endDate) > new Date()).length}
                  </p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">90-Day Suspensions</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Total Suspensions</p>
            ...
            {filteredSuspensions.length === 0 ? (
              ...
              <div className="space-y-4">
                {filteredSuspensions.map((suspension) => (
                  <div key={suspension._id} className="border rounded-lg p-4 hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <Badge variant="outline" className={
                            suspension.type === "90-day" ? "border-red-500 text-red-700" : "border-orange-500 text-orange-700"
                          }>
                            {suspension.type}
                          </Badge>
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          <div className="flex items-center gap-1">
                            <Calendar size={12} />
                            <span>
                              {format(new Date(suspension.startDate), 'MMM dd, yyyy')} - {format(new Date(suspension.endDate), 'MMM dd, yyyy')}
                            </span>
                          </div>
                          {suspension.isActive && new Date(suspension.endDate) > new Date() && (
                            <div className="flex items-center gap-1">
                              <Clock size={12} />
                              <span>{getDaysRemaining(suspension.endDate)} days remaining</span>
                            </div>
                          )}
                        </div>
                      </div>
