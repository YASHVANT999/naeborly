...
export default function DMTracking() {
  ...
  // Fetch company DMs
  const { data: companyDMs = [], isLoading: loadingDMs, refetch: refetchDMs } = useQuery({
  ...
  const handleReplaceDM = () => {
    ...
    replaceDMMutation.mutate({ originalDMId: selectedDM.dmId, replacementDMId });
  };
  ...
  const getVerificationBadge = (status) => {
    switch (status) {
      case 'verified':
        return <Badge variant="default" className="bg-green-100 text-green-800"><CheckCircle className="mr-1 h-3 w-3" />Verified</Badge>;
      case 'rejected':
        return <Badge variant="destructive"><XCircle className="mr-1 h-3 w-3" />Rejected</Badge>;
      case 'suspended':
        return <Badge variant="secondary" className="bg-red-100 text-red-800"><AlertTriangle className="mr-1 h-3 w-3" />Suspended</Badge>;
      default:
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">
                  {companyDMs.filter(dm => dm.verificationStatus === 'verified').length}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">
                  {companyDMs.length > 0 ? 
                    Math.round(companyDMs.reduce((sum, dm) => sum + dm.engagementScore, 0) / companyDMs.length) : 
                    0
                  }
                </p>
              </div>
        ...
        <CardContent>
          {companyDMs.length > 0 ? (
                ...
                <TableBody>
                  {companyDMs.map((dm) => (
                    <TableRow key={dm.id}>
                      <TableCell className="font-medium">
                        <div className="flex items-center space-x-2">
                          <div>
                            ...
                            <div className="text-xs text-gray-400">{dm.company}</div>
                          </div>
                          {dm.linkedinUrl && (
                            <a 
                              href={dm.linkedinUrl} 
                      ...
                      <TableCell>{getVerificationBadge(dm.verificationStatus)}</TableCell>
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          {dm.flagCount > 0 ? (
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          <Star className={`h-4 w-4 ${getEngagementColor(dm.engagementScore)}`} fill="currentColor" />
                          <span className={`font-medium ${getEngagementColor(dm.engagementScore)}`}>
                            {dm.engagementScore}
                          </span>
                        </div>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{dm.linkedRep.email}</div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">{formatDate(dm.referralDate)}</div>
                        {dm.lastInteraction && (
                          <div className="text-xs text-gray-500">
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() => updateVerificationMutation.mutate({
                                dmId: dm.dmId,
                                verificationStatus: dm.verificationStatus === 'verified' ? 'pending' : 'verified'
                              })}
                            >
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-semibold text-gray-900">No decision makers</h3>
              <p className="mt-1 text-sm text-gray-500">
                Decision makers will appear here when your sales reps start making referrals.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={isRemovalDialogOpen} onOpenChange={setIsRemovalDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Provide a reason for requesting the removal of {selectedDM?.name}. This action will be reviewed.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isReplaceDialogOpen} onOpenChange={setIsReplaceDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Replace {selectedDM?.name} with a new decision maker. Enter the ID of the replacement DM.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select value={flagForm.flagType} onValueChange={(value) => setFlagForm(prev => ({ ...prev, flagType: value }))}>
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="scheduling_issues">Scheduling Issues</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <Select value={flagForm.severity} onValueChange={(value) => setFlagForm(prev => ({ ...prev, severity: value }))}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
