...
export default function PerformanceAnalytics() {
  ...
  const { overview = {}, topPerformers = {}, trends = {}, distributions = {}, repPerformance = [] } = analytics;

  const handleExport = async () => {
    setIsExporting(true);
    try {
      const response = await fetch(`/api/company-analytics/export?type=${exportType}`, {
        credentials: 'include'
      });
  ...
  const callOutcomeData = distributions.callOutcomes ? [
    { name: 'Completed', value: distributions.callOutcomes.completed, fill: colors.success },
    { name: 'Missed', value: distributions.callOutcomes.missed, fill: colors.danger },
    { name: 'Cancelled', value: distributions.callOutcomes.cancelled, fill: colors.warning },
    { name: 'Scheduled', value: distributions.callOutcomes.scheduled, fill: colors.secondary }
  ] : [];

  const dmVerificationData = distributions.dmVerification ? [
    { name: 'Verified', value: distributions.dmVerification.verified, fill: colors.success },
    { name: 'Pending', value: distributions.dmVerification.pending, fill: colors.warning },
    { name: 'Rejected', value: distributions.dmVerification.rejected, fill: colors.danger },
    { name: 'Suspended', value: distributions.dmVerification.suspended, fill: colors.muted }
  ] : [];
  ...
  const getPerformanceIcon = (value, type) => {
    if (type === 'feedback') {
      if (value >= 4.5) return <CheckCircle className="h-5 w-5 text-green-600" />;
      if (value >= 3.5) return <AlertTriangle className="h-5 w-5 text-yellow-600" />;
      return <AlertTriangle className="h-5 w-5 text-red-600" />;
    }
    if (type === 'engagement') {
      if (value >= 80) return <TrendingUp className="h-5 w-5 text-green-600" />;
      if (value >= 60) return <TrendingUp className="h-5 w-5 text-yellow-600" />;
      return <TrendingUp className="h-5 w-5 text-red-600" />;
    }
    if (type === 'noshow') {
      if (value <= 10) return <CheckCircle className="h-5 w-5 text-green-600" />;
      if (value <= 25) return <AlertTriangle className="h-5 w-5 text-yellow-600" />;
      return <AlertTriangle className="h-5 w-5 text-red-600" />;
    }
    return null;
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Avg Rep Feedback</p>
                <div className="flex items-center space-x-2 mt-1">
                  <p className={`text-2xl font-bold ${getPerformanceColor(overview.avgRepFeedbackScore, 'feedback')}`}>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Avg DM Engagement</p>
                <div className="flex items-center space-x-2 mt-1">
                  <p className={`text-2xl font-bold ${getPerformanceColor(overview.avgDMEngagementScore, 'engagement')}`}>
                    {overview.avgDMEngagementScore || 0}
                  </p>
                  {getPerformanceIcon(overview.avgDMEngagementScore, 'engagement')}
                </div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <p className={`text-2xl font-bold ${getPerformanceColor(overview.noShowRate, 'noshow')}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Call Volume Trend */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-3">Call Volume & Success Rate</h4>
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={trends.monthly}>
                    ...
                    <Bar dataKey="completedCalls" fill={colors.success} name="Completed" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              {/* Feedback Trend */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-3">Average Feedback Score</h4>
                <ResponsiveContainer width="100%" height={250}>
                ...
                {topPerformers.byCalls.slice(0, 5).map((rep, index) => (
                  <div key={rep.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-blue-600">{rep.completedCalls}</p>
              ...
              <div className="space-y-3">
                {topPerformers.byFeedback.slice(0, 5).map((rep, index) => (
                  <div key={rep.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-yellow-600">{rep.avgFeedback.toFixed(1)}</p>
              ...
              <div className="space-y-3">
                {topPerformers.byDMInvites.slice(0, 5).map((rep, index) => (
                  <div key={rep.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-green-600">{rep.dmInvites}</p>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={repPerformance} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis 
                  dataKey="name" 
                  angle={-45}
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="completedCalls" fill={colors.primary} name="Completed Calls" />
                <Bar dataKey="dmInvites" fill={colors.secondary} name="DM Invites" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{overview.totalCalls || 0}</p>
              <p className="text-sm text-gray-600">Total Calls</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">{overview.totalDMs || 0}</p>
              <p className="text-sm text-gray-600">Decision Makers</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">{overview.totalReps || 0}</p>
              <p className="text-sm text-gray-600">Sales Reps</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-600">{overview.completionRate || 0}%</p>
