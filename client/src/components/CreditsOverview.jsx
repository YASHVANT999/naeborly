...
export default function CreditsOverview() {
  ...
  const { data: creditsData, isLoading: loadingCredits, refetch: refetchCredits } = useQuery({
  ...
  const updateLimitsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      refetchCredits();
      queryClient.invalidateQueries({ queryKey: ['/api/company-credits/summary'] });
    },
    onError: (error) => {
      toast({
        title: "Error",
  ...
  const handleUpdateLimits = (e) => {
    e.preventDefault();
    const limits = {
      maxCallsPerMonth: limitsForm.maxCallsPerMonth ? parseInt(limitsForm.maxCallsPerMonth) : null,
      maxDMsPerMonth: limitsForm.maxDMsPerMonth ? parseInt(limitsForm.maxDMsPerMonth) : null
    };
    updateLimitsMutation.mutate(limits);
  };

  const handleResetCredits = () => {
    if (window.confirm('Are you sure you want to reset all credits and usage statistics? This action cannot be undone.')) {
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">{monthlyCredits.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Current billing period: {currentPeriodStart && formatDate(currentPeriodStart)} - {currentPeriodEnd && formatDate(currentPeriodEnd)}
              </CardDescription>
            </div>
                ...
                <DialogContent>
                  <form onSubmit={handleUpdateLimits}>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Set monthly limits for each sales representative. Leave empty for unlimited access.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setLimitsForm(prev => ({ ...prev, maxCallsPerMonth: e.target.value }))}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="maxDMs">Max Decision Makers Per Month</Label>
                        <Input
                          id="maxDMs"
                          type="number"
                          placeholder="Unlimited"
                          value={limitsForm.maxDMsPerMonth}
                          onChange={(e) => setLimitsForm(prev => ({ ...prev, maxDMsPerMonth: e.target.value }))}
                        />
                      </div>
                    </div>
                    <DialogFooter>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
              ...
              <div className="text-center p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">Decision Makers Unlocked</p>
              </div>
              <div className="text-center p-4 bg-gray-50 rounded-lg">
          ...
          {repUsage.length > 0 ? (
            <div className="rounded-md border">
              <Table>
                ...
                <TableBody>
                  {repUsage.map((rep) => {
                    const callsStatus = getUsageStatus(rep.callsBooked, perRepLimits.maxCallsPerMonth);
                    const dmsStatus = getUsageStatus(rep.dmsUnlocked, perRepLimits.maxDMsPerMonth);
                    ...
                    return (
                      <TableRow key={rep.repId}>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            ...
                            {perRepLimits.maxCallsPerMonth && (
                              <Badge variant="outline" className={getStatusColor(callsStatus)}>
                                /{perRepLimits.maxCallsPerMonth}
                              </Badge>
                            )}
                          </div>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            ...
                            {perRepLimits.maxDMsPerMonth && (
                              <Badge variant="outline" className={getStatusColor(dmsStatus)}>
                                /{perRepLimits.maxDMsPerMonth}
                              </Badge>
                            )}
                          </div>
                        ...
                        <TableCell>
                          {(callsStatus === 'critical' || dmsStatus === 'critical') ? (
                            <Badge variant="destructive">Limit Reached</Badge>
                          ) : (callsStatus === 'warning' || dmsStatus === 'warning') ? (
                            <Badge variant="outline" className="text-yellow-600 bg-yellow-100">Near Limit</Badge>
                          ) : (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Usage statistics will appear here once your sales reps start booking calls.
              </p>
            </div>
          )}
        </CardContent>
