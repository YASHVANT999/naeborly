...
export default function EnterpriseProtectedRoute({ children }) {
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      if (user?.role !== 'enterprise_admin') {
        toast({
          title: "Access Denied",
          description: "Enterprise admin privileges required to access this page",
          variant: "destructive",
        });
      ...
      if (!user?.companyDomain || !user?.domainVerified) {
        toast({
          ...
          description: "Your company domain must be verified for enterprise access",
          variant: "destructive",
        });
        setLocation('/');
        return;
  ...
  if (isAuthenticated && user?.role === 'enterprise_admin' && user?.domainVerified) {
    return children;
  }
