...
export default function AccountSettings() {
  ...
  const billingPortalMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "You've been redirected to manage your billing information",
      });
    },
    onError: (error) => {
      toast({
  ...
  const getPlanStatusBadge = (status) => {
    switch (status) {
      ...
      case 'trial':
        return <Badge variant="secondary"><AlertCircle className="mr-1 h-3 w-3" />Trial</Badge>;
      case 'cancelled':
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                <Label className="text-sm font-medium text-gray-600">Verified Domain</Label>
                <div className="mt-1 flex items-center space-x-2">
                  <Badge variant="default" className="bg-green-100 text-green-800">
                    <Shield className="mr-1 h-3 w-3" />
                    Verified
                  </Badge>
                  <span className="text-sm text-gray-700">{company.verifiedDomain}</span>
                </div>
              </div>
              <div>
                <Label className="text-sm font-medium text-gray-600">Team Size</Label>
                <div className="mt-1 text-sm text-gray-700">
                  {company.totalUsers} total users ({company.salesReps} sales reps, {company.decisionMakers} decision makers)
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 space-y-1">
                  <div className="font-medium">{company.adminContact?.name}</div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {formatCurrency(plan.pricing?.basePrice)} / {plan.billingCycle}
                  </p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span>{formatDate(plan.currentPeriodStart)} - {formatDate(plan.currentPeriodEnd)}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Next Renewal:</span>
                  <span className="font-medium">{formatDate(plan.renewalDate)}</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t">
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  <div>Monthly Call Credits: {plan.limits?.monthlyCallCredits}</div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">{usage.currentMonth?.calls || 0}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">DMs Referred</span>
                <span className="font-medium">{usage.currentMonth?.dmsReferred || 0}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Credits Used</span>
                <span className="font-medium">{usage.currentMonth?.creditUsage || 0}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Credits Remaining</span>
                <span className="font-medium text-green-600">{usage.currentMonth?.remainingCredits || 0}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Account Manager */}
            <div className="space-y-4">
              <h5 className="font-medium text-gray-900">Your Account Manager</h5>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h6 className="font-medium">{support.accountManager?.name}</h6>
