...
export default function CallActivityLog() {
  ...
  }, [selectedRep, selectedDM, selectedOutcome, startDate, endDate, searchTerm]);

  // Fetch call logs with filters
  const { data: callLogs = [], isLoading: loadingCalls, refetch: refetchCalls } = useQuery({
  ...
  const getOutcomeBadge = (status, flagged) => {
    if (flagged) {
      return <Badge variant="destructive"><Flag className="mr-1 h-3 w-3" />Flagged</Badge>;
    }
    ...
    switch (status) {
      ...
      case 'scheduled':
        return <Badge variant="outline"><Calendar className="mr-1 h-3 w-3" />Scheduled</Badge>;
      default:
          ...
          {sortedCalls.length > 0 ? (
            <div className="rounded-md border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="cursor-pointer" onClick={() => handleSort('repToDM')}>
                      <div className="flex items-center">
                        Rep ↔ DM
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </div>
                    </TableHead>
                    <TableHead className="cursor-pointer" onClick={() => handleSort('scheduledAt')}>
                      <div className="flex items-center">
                        Date/Time
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </div>
                    ...
                    <TableHead className="cursor-pointer" onClick={() => handleSort('feedback.rating')}>
                      <div className="flex items-center">
                        Rating
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </div>
                      ...
                      <TableRow key={call.id}>
                          ...
                          <div className="space-y-1">
                            ...
                            {call.completedAt && (
                              <div className="text-xs text-green-600">
                                Completed: {formatDateTime(call.completedAt).time}
                              </div>
                            )}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="max-w-xs">
                            <div className="text-sm truncate" title={call.feedback?.summary}>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-semibold text-gray-900">No call activity</h3>
              <p className="mt-1 text-sm text-gray-500">
                Call activity will appear here when your team starts scheduling meetings.
              </p>
            </div>
          )}
        </CardContent>
