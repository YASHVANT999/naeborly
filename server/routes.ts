...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Initialize default flag rules
  app.post("/api/flag-rules/initialize", requireSuperAdmin, async (req, res) => {
    try {
      const defaultRules = [
        {
          name: "Missed Follow-ups Threshold",
          description: "Too many missed follow-ups with decision makers",
          userType: "sales_rep",
        ...
        {
          name: "Invalid Leads Threshold", 
          description: "Too many invalid or low-quality leads submitted",
          userType: "sales_rep",
          metric: "invalidLeads",
          thresholds: { amber: 2, red: 4 },
          timeWindow: 30,
          severity: "high"
        },
        {
          name: "Compliance Score Threshold",
          description: "Low compliance score indicating poor performance",
          userType: "sales_rep", 
          metric: "complianceScore",
          thresholds: { amber: 70, red: 50 },
          timeWindow: 30,
          severity: "high"
        },
        {
          name: "Response Rate Threshold",
          description: "Low response rate to invitations and communications",
      ...
      const { FlagRule } = await import('./mongodb');
      await FlagRule.deleteMany({}); // Clear existing rules
      await FlagRule.insertMany(defaultRules);

      res.json({ success: true, message: "Flag rules initialized", rulesCount: defaultRules.length });
    } catch (error) {
      console.error('Error initializing flag rules:', error);
      res.status(500).json({ message: "Failed to initialize flag rules" });
    }
  });

  // Get user's current flag status
  app.get("/api/user-flag/:userId", requireAuthentication, async (req, res) => {
    try {
      const { userId } = req.params;
      const currentUserId = (req.session as any).userId;
      
      // Users can only access their own flag or super admin can access any
      if (currentUserId !== userId && (req.session as any).userRole !== 'super_admin') {
        return res.status(403).json({ message: "Access denied" });
      }

      let userFlag = await storage.getUserFlag(userId);
      ...
      if (!userFlag) {
        ...
        // Initialize flag for new user
        userFlag = await storage.initializeUserFlag(userId, user.role);
      }

      res.json(userFlag);
    } catch (error) {
      console.error('Error getting user flag:', error);
      res.status(500).json({ message: "Failed to get user flag" });
    }
  });

  // Calculate and update user risk
  app.post("/api/user-flag/:userId/calculate", requireAuthentication, async (req, res) => {
    try {
      const { userId } = req.params;
      const currentUserId = (req.session as any).userId;
      
      // Users can only calculate their own risk or super admin can calculate any
      if (currentUserId !== userId && (req.session as any).userRole !== 'super_admin') {
        return res.status(403).json({ message: "Access denied" });
      }
      ...
      const riskCalculation = await storage.calculateUserRisk(userId, user.role);
      res.json(riskCalculation);
    } catch (error) {
      console.error('Error calculating user risk:', error);
      res.status(500).json({ message: "Failed to calculate user risk" });
    }
  });

  // Get flag history for user
  app.get("/api/user-flag/:userId/history", requireAuthentication, async (req, res) => {
    try {
      const { userId } = req.params;
      const limit = parseInt(req.query.limit as string) || 10;
      const currentUserId = (req.session as any).userId;
      
      // Users can only access their own history or super admin can access any
      if (currentUserId !== userId && (req.session as any).userRole !== 'super_admin') {
        return res.status(403).json({ message: "Access denied" });
      }

      const history = await storage.getFlagHistory(userId, limit);
      res.json(history);
    } catch (error) {
      console.error('Error getting flag history:', error);
      res.status(500).json({ message: "Failed to get flag history" });
    }
  });

  // Get flags for all decision makers referred by a sales rep
  app.get("/api/sales-rep/:salesRepId/decision-maker-flags", requireAuthentication, async (req, res) => {
    try {
      const { salesRepId } = req.params;
      const currentUserId = (req.session as any).userId;
      
      // Sales rep can only access their own referred DMs or super admin can access any
      if (currentUserId !== salesRepId && (req.session as any).userRole !== 'super_admin') {
        return res.status(403).json({ message: "Access denied" });
      }

      // Get all invitations sent by this sales rep that were accepted
      const acceptedInvitations = await storage.getInvitationsByUserId(salesRepId);
      const acceptedDMs = acceptedInvitations.filter(inv => inv.status === 'accepted');
      
      const dmFlags = [];
      for (const invitation of acceptedDMs) {
        // Find decision maker user by email
        const dm = await storage.getUserByEmail(invitation.decisionMakerEmail);
        if (dm) {
          let dmFlag = await storage.getUserFlag(dm.id);
          if (!dmFlag) {
            dmFlag = await storage.initializeUserFlag(dm.id, 'decision_maker');
          }
      ...
      console.error('Error getting decision maker flags:', error);
      res.status(500).json({ message: "Failed to get decision maker flags" });
    }
  });
